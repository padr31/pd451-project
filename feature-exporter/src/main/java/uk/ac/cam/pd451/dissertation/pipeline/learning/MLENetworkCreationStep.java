package uk.ac.cam.pd451.dissertation.pipeline.learning;
...
public class MLENetworkCreationStep implements Step<List<Clause>, ProvenanceGraph> {
    ...
    public ProvenanceGraph process(List<Clause> groundClauses) throws PipeException {
        ...
        Map<String, Double> ruleProbabilities = CSV.readRuleProbabilitiesCSV();
        ...
        clauseToNode.forEach((cl, node) -> {
            ...
            Map<Assignment, Double> function = Assignment.allAssignments(assignmentVariables).stream().collect(Collectors.toMap(a -> a, a -> {
                ...
                double ruleProb = 0.95;
                if(ruleProbabilities.containsKey(cl.getFullRule())) {
                    counter[0]++;
                    ruleProb = ruleProbabilities.get(cl.getFullRule());
                }
        ...
        System.out.println(counter[0] + " learned predicates set");
